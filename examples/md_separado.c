	     ...
	     double *pot_p, real8 *kin_p) 
{
 ...
 #pragma omp parallel  shared(f, vel, pos, np, nd, box, pot) private(k, rij, d, j, i)   
  {
...
void update(int np, int nd, vnd_t pos[nparts], vnd_t vel[nparts], vnd_t f[nparts], vnd_t a[nparts],  double mass, real8 dt)
{
  int i, j;
  double rmass;
